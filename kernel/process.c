...
int do_fork(process *parent)
{
  ...
  for (int i = 0; i < parent->total_mapped_region; i++)
  {
    ...
    case CODE_SEGMENT:
      ...
      {
        void *child_pa = (void *)lookup_pa(parent->pagetable, parent->mapped_info[i].va);
...
{
  struct dentry *now = den;
  int len = 0;
  if (now == vfs_root_dentry)
  {
    path[0] = '/';
    path[1] = '\0';
    return 0;
...
{
  int len = strlen(ori_path);
  if (len == 0 || ori_path[0] == '/')
  {
    strcpy(out_path, ori_path);
    return 0;
  }
  struct dentry *den = proc->pfiles->cwd;
  if (ori_path[0] == '.')
  {
    if (len == 1 || ori_path[1] == '/')
    {
    ...
    else if (ori_path[1] == '.' && (len == 2 || ori_path[2] == '/'))
    {
      if (den->parent != NULL)
      {
        den = den->parent;
  ...
  if (get_path(den, out_path) != 0)
  {
    return -1;
  }
  strcat(out_path, ori_path);
  return 0;
}